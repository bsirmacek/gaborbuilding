...
function lc = showkeys(image, locs)
...
for i = 1: size(locs,1)
    ...
    lc(i,:) = TransformLine(imsize, locs(i,:), 0.0, 0.0, 1.0, 0.0);
    l = TransformLine(imsize, locs(i,:), 0.85, 0.1, 1.0, 0.0);
    l = TransformLine(imsize, locs(i,:), 0.85, -0.1, 1.0, 0.0);
end
hold off;
...
function lc = TransformLine(imsize, keypoint, x1, y1, x2, y2)
